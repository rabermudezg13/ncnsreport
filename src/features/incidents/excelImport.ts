...
const timeValue=(value:Cell)=>{if(value instanceof Date)return `${pad(value.getHours())}:${pad(value.getMinutes())}`;if(typeof value==="number"){const minutes=Math.round((value-Math.floor(value))*1440)%1440;return `${pad(Math.floor(minutes/60))}:${pad(minutes%60)}`}const text=normalize(value);const match=text.match(/(?:T|\s)(\d{1,2}):(\d{2})/)??text.match(/^(\d{1,2}):(\d{2})/);return match?`${pad(Number(match[1]))}:${match[2]}`:""};
