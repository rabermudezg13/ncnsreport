...
export interface ImportRow {rowNumber:number;sourceRecordId:string;confirmationNumber:string;firstName:string;lastName:string;email:string;ksnNumber:string;schoolName:string;district:string;state:string;incidentDate:string;scheduledTime:string;incidentType:IncidentType|null;sourceNotes:string;errors:string[]}
...
export async function parseIncidentWorkbook(file:File){
  ...
  return rows.slice(1).filter(row=>row.some(Boolean)).map((row,rowIndex):ImportRow=>{
    const parsedName=names(normalize(row[columns.name]));const type=incidentType(normalize(row[columns.type]));const district=columns.district>=0?normalize(row[columns.district]):"";const state=columns.state>=0?normalize(row[columns.state]):"";const schoolName=normalize(row[columns.school])||"School not provided";const incidentDate=dateValue(row[columns.date]);const scheduledTime=columns.created>=0?timeValue(row[columns.created]):"";const errors:string[]=[];
    if(!parsedName.firstName||!parsedName.lastName)errors.push("Full substitute name is required");if(!incidentDate)errors.push("Valid incident date is required");if(!type)errors.push("Incident type must be No Call No Show or Late Cancellation");
    const details=[["Channel",columns.channel>=0?normalize(row[columns.channel]):""]].filter(([,value])=>value).map(([label,value])=>`${label}: ${value}`).join(" · ");
    return{rowNumber:rowIndex+2,sourceRecordId:columns.record>=0?normalize(row[columns.record]):"",confirmationNumber:columns.confirmation>=0?normalize(row[columns.confirmation]):"",...parsedName,email:columns.email>=0?normalize(row[columns.email]):"",ksnNumber:columns.ksn>=0?normalize(row[columns.ksn]).replace(/^\(/,""):"",schoolName,district,state,incidentDate,scheduledTime,incidentType:type,sourceNotes:details,errors};
  });
}
